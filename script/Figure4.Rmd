...
```{r}
Human_TP <- Result_mix_TP_final %>% subset(group =="Human")
```

```{r}
library(propagate)

Human_TP <- Human_TP %>% subset(Sample_ID =="JAB14")
DAT <- data.frame (Marine_percent = c(Human_TP$Mean_marine, Human_TP$SD_marine), Glx = c(Human_TP$glu_d15N, Human_TP$glu_d15Nstdev), Phe = c(Human_TP$phe_d15N, Human_TP$phe_d15Nstdev), Beta_M = c(3.3, 1.8), Beta_T = c(-6.6, 3.4), TDF = c(6.6, 1.7))
DAT <- as.matrix(DAT)
EXPR <- expression (((Glx - Phe) - ((Marine_percent*Beta_M) + ((1-Marine_percent)*Beta_T))) / TDF + 1)
RESJAB14 <- propagate(expr = EXPR, data = DAT, type = "stat", 
...
```{r}
EXPR <- expression (TP == ((Glx - Phe) - ((Marine_percent*Beta_M) + ((1-Marine_percent)*Beta_T))) / TDF + 1)
prop_result <- propagate(EXPR, DAT)
Result_mix_TP_final$z <- prop_result$prop[, "Mean"]
Result_mix_TP_final$z.err <- prop_result$prop[, "sd"]

 Propagate directly from the dataframe
prop_result <- propagate(
  expr,
  data = df[, c("x", "y")],
  errs = df[, c("x.err", "y.err")],
  do.sim = FALSE
)
