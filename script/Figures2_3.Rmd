...
```{r}
Fig2c <-ggplot() +
  geom_point(data=fauna, aes(y=gluphe_d15N,  x=valphe_d13C, fill=category), colour="black", pch=23,alpha=0.6, size=4)+
...
```{r}
Fig3d <-ggplot() +
  geom_point(data=fauna, aes(y=lysphe_d15N,  x=valphe_d13C, colour=category), alpha=0.8, pch=23, size=4)+
  geom_point(data=humans, aes(y=lysphe_d15N,  x=valphe_d13C, fill=Site), colour="black", pch=21, size=4)+
  labs(y=expression(Delta^{15}*N[Lys-Phe]*"(\u2030)"), x=expression(Delta^{13}*C[Val-Phe]*"(\u2030)"))+
 scale_fill_manual(values=c(cols), breaks=c(order))+
  scale_colour_manual(values=c(cols), breaks=c(order))+
  scale_x_continuous(position="bottom", limits=c(-5,8))+
